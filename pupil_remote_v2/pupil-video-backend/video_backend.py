...
class VideoBackEnd:
    ...
    def start(self, device="world", callback=None):
        ...
        try:
            """
            topic="notify."+"eye_process.should_start.0"
            n = {'subject':'eye_process.should_start.0','eye_id': 0, 'args':{"topics": (topic,), "hwm": 1}}
            print(self.pupil.notify(n))
            topic="notify."+"eye_process.should_start.1"
            n = {'subject':'eye_process.should_start.1','eye_id': 1, 'args':{"topics": (topic,), "hwm": 1}}
            print(self.pupil.notify(n))"""
            self.device = device
    ...
    def _listenAndStartStreaming(self, callback):
        ...
        try:
            if self.device == "eye0":
                topic="notify."+"eye_process.should_start.0"
                n = {'subject':'eye_process.should_start.0','eye_id': 0, 'args':{"topics": (topic,), "hwm": 1}}
                print("eye0" +str(self.pupil.notify(n)))
            if self.device == "eye1":
                topic="notify."+"eye_process.should_start.1"
                n = {'subject':'eye_process.should_start.1','eye_id': 1, 'args':{"topics": (topic,), "hwm": 1}}
                print("eye1" +str(self.pupil.notify(n)))
            if self.device == "world":
            ...
            while listen_to_notification:
                ...
                if True or (b"eye_process.started" == message[b"subject"] and self.device[
                    -1
    ...
    def _streamVideo(self):
        try:
            ...
            ipras="127.0.0.1"
            if self.device=="world":
                source = f"http://{ipras}:8083/?action=stream"
            if self.device=="eye0":
                source= f"http://{ipras}:8081/?action=stream"
            if self.device=="eye1":
                source= f"http://{ipras}:8082/?action=stream"
